...
public class Player extends GameObject implements Damagable, DamageInflicter {
	...
	public void keyDownEvent(GameKeyCode key) {
		...
		switch (key) {
		case A:
			if (!getRoom().inCollision(this, getX() - 5, getY(), true)
					&& getRoom().isInRoom(getX() - 5, getY(), this)) {
				setX(getX() - 5);
			}
			break;
		case D:
			...
			break;
		case W:
			if (!getRoom().inCollision(this, getX(), getY() - 5, true)
					&& getRoom().isInRoom(getX(), getY() - 5, this)) {
				setY(getY() - 5);
			}
			break;
		case S:
